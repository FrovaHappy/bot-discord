...
export function send_withoutParameters(interaction, countdownQuery) {
  ...
  `
  const embed = new EmbedBuilder().setTitle('Nada para hacer:').setColor('#4990ff').setDescription(description)
...
export function send_dataSaved(interaction, countdownQuery) {
  let description = `
    ...
    ${buildersDbChange(countdownQuery)}
  `
  const embed = new EmbedBuilder().setTitle('Datos guardados:').setColor('#4990ff').setDescription(description)
  interaction.reply({ embeds: [embed] })
}

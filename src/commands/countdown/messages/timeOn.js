...
export function send_OverflowValue(interaction, time) {
  ...
  `
  const embed = new EmbedBuilder().setTitle('Tiempo Invalido:').setColor('#FFA233').setDescription(description)
...
export function send_timeInit(interaction, time, messageId) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('timeDeleted').setLabel('Stop.').setStyle('DANGER')
  )

...
export async function putApplicationGuildsCommands( applicationId, guildsIds, commands) {
  ...
  guildsIds.map((guildId) => {
    ...
    result.push(
      rest.put(fullroute, { body: commands } )
        .then( 
          (_resolve) => { return { guildId, done: true, commands };},
          (_reject) => { return { guildId, done: false, commands };}
        )
    )
  })
...
export async function putApplicationCommands( applicationId, commands ) {
  ...
  const result = await rest.put( fullroute, { body: commands,} )
    .then (
      (_resolve) => { return {done: true, commandsName} },
      (_reject) => { return {done: false, commandsName} }
    )
  return result 
}
/**
...
export async function deleteForCommandId( fullroute ) {
  ...
  query.map( command => {
    ...
    result.push(
      rest.delete(fullrouteCommandId)
        .then (
          (_resolve) => { 
            count++
            return { name: command.name, ok: 1 }},
          (_reject) => { return { name: command.name, ok: 0 }}
        )
    )
  })
  result = await Promise.all(result)
